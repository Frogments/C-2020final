...
int main()
{
	...
	for (int i = 0; i < num; ++i) {
		...
		// 여기에 들어갈 코드를 답지에 적는다. ~10
		if (sel == 0)
			gameObjects[i] = new Player;
		else if (sel == 1)
			gameObjects[i] = new Monster;
		else
	...
	int numOfTree{ 0 };
	for (int i = 0; i < num; ++i) {
		if (dynamic_cast<Tree*>(gameObjects[i]) != nullptr)
			++numOfTree;
	}
	// 여기에 들어갈 코드를 답지에 적는다. ~15
...
void f(Game**& gameObjects, int n) {
	for (int i = 0; i < n; ++i)
		if (dynamic_cast<Player*>(gameObjects[i]) != nullptr)
			if (dynamic_cast<Player*>(gameObjects[i])->getNation())
				dynamic_cast<Player*>(gameObjects[i])->setHp(0);
}
